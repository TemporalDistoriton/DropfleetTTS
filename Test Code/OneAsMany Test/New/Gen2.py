...
def update_lua_script_state(state_json: str, row: dict) -> str:
    ...
    # Special rules flags (only set if the keys already exist in the saved state)
    special_text = str(row.get('Special', '') or '').lower()
    has_vectored = 'vectored' in special_text
    has_monitor = 'monitor' in special_text
    if 'Vectored' in state:
        state['Vectored'] = has_vectored
    if 'Monitor' in state:
        state['Monitor'] = has_monitor
...
def create_ship_from_template(template: dict, row: dict, existing_guids: set) -> dict:
    ...
    # Special rules -> boolean locals in the ship Lua script
    special_text = str(row.get('Special', '') or '').lower()
    has_vectored = 'vectored' in special_text
    has_monitor = 'monitor' in special_text
    lua = replace_lua_variable(lua, 'Vectored', str(has_vectored).lower())
    lua = replace_lua_variable(lua, 'Monitor', str(has_monitor).lower())
