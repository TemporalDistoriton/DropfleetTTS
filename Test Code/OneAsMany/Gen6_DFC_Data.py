#!/usr/bin/env python3
"""
TTS Ship Generator v6 — Template-Optimised (with Spawner Tiles)
================================================================
...
def art_url(pdf_id: str, asset_name: str, suffix: str) -> str:
    """Build an art URL using the same faction/name layout as the old sheet."""
    return f"{ART_BASE_URL}/{pdf_id}/{asset_name}{suffix}"
...
def famous_admiral_row(data: dict, pdf_id: str) -> dict:
    ...
    admiral = str(data.get('Admiral', '')).strip()
...
def load_index_records(folder: Path, index: dict, key: str, builder, pdf_id=None) -> list:
    """Load every filename listed under one index key."""
    rows = []
    filenames = index.get(key, []) or []
...
def load_fleet_folder(folder: Path) -> list:
    ...
    rows = []
    rows.extend(load_index_records(folder, index, 'Ships', ship_row, pdf_id))
    rows.extend(load_index_records(folder, index, 'Heroes', hero_row, pdf_id))
    rows.extend(load_index_records(folder, index, 'FamousAdmirals', famous_admiral_row, pdf_id))
    rows.extend(load_index_records(folder, index, 'DeployableFeatures', deployable_feature_row, pdf_id))
    return rows


def load_civilian_folder(folder: Path) -> list:
    """Load civilian ship records through _civilian.json."""
    index = load_json(folder / '_civilian.json')
    return load_index_records(folder, index, 'Ships', ship_row, 'CIV')


def load_stations_folder(folder: Path) -> list:
    """Load fleet-specific stations; generic stations have no faction container."""
    index = load_json(folder / '_stations.json')
    return load_index_records(folder, index, 'FleetStations', station_row)


def read_ship_rows_from_data(data_directory: Path) -> list:
    """Build the former spreadsheet rows directly from dfc-data JSON files."""
    rows = []
    loaded_sources = []

    for folder in sorted((p for p in data_directory.iterdir() if p.is_dir()),
                         key=lambda p: p.name.casefold()):
        if (folder / '_fleet.json').is_file():
            faction_rows = load_fleet_folder(folder)
            rows.extend(faction_rows)
            loaded_sources.append((folder.name, len(faction_rows)))
        elif (folder / '_civilian.json').is_file():
            civilian_rows = load_civilian_folder(folder)
            rows.extend(civilian_rows)
            loaded_sources.append((folder.name, len(civilian_rows)))
        elif (folder / '_stations.json').is_file():
            station_rows = load_stations_folder(folder)
            rows.extend(station_rows)
            loaded_sources.append((folder.name, len(station_rows)))

    if not loaded_sources:
        raise DataImportError(
            f"No _fleet.json, _civilian.json, or _stations.json indexes were found in {data_directory}"
        )

    # Protect against an index listing the exact same record twice, while
    # preserving legitimate same-name variants with different profiles.
    unique_rows = []
    seen = set()
    for row in rows:
    ...
    for source, count in loaded_sources:
        print(f"  {source}: {count} ship-like records")
