#!/usr/bin/env python3
"""
TTS Ship Generator v19 — Template-Optimised (with Full Ship Data)
================================================================
...
Artwork names are resolved from the repository's local RemasterShips folder,
which is located automatically by walking upward from this script.
...
SCRIPT_VERSION = "Gen19 repository-wide ship discovery build 2026-08-17"
...
# --- Full source-data payload -------------------------------------------------

SHIP_DATA_SCHEMA_VERSION = 1


def lua_long_string_literal(text: str, start_level: int = 4) -> str:
    """Return *text* as a Lua long-string literal with a safe delimiter."""
    value = str(text or '')
    equals = '=' * start_level
...
def set_source_path_metadata(row: dict, source_path) -> dict:
    """Attach portable source-file metadata to a converted row."""
    if source_path is None:
        return row
    path = Path(source_path)
    row['Source File'] = path.name
    row['Source Folder'] = path.parent.name
    return row


def build_ship_data_payload(row: dict) -> dict:
    """Build the runtime data table exposed to ship and feature Lua.

    The source record is preserved verbatim. A reserved ``_TTS`` metadata
    block is added so later XML code can identify the record consistently
    without needing to infer whether it is a Ship, Hero, Famous Admiral,
    Deployable Feature, Station, or Civilian Ship.
    """
    source = row.get('Source Data', {})
    payload = copy.deepcopy(source) if isinstance(source, dict) else {}
    payload['_TTS'] = {
        'SchemaVersion': SHIP_DATA_SCHEMA_VERSION,
        'RecordKind': str(row.get('Record Kind', 'Ship') or 'Ship'),
        'Faction': str(row.get('PDF ID', '') or ''),
        'DisplayName': str(row.get('Name', '') or ''),
        'FilterType': str(row.get('Filter Type', row.get('Type', 'Other')) or 'Other'),
        'SourceFile': str(row.get('Source File', '') or ''),
        'SourceFolder': str(row.get('Source Folder', '') or ''),
    }
    return payload


def build_ship_data_json(row: dict) -> str:
    """Serialise the full source record compactly for embedding in Lua."""
    return json.dumps(
        build_ship_data_payload(row),
        ensure_ascii=False,
        separators=(',', ':'),
    )
...
# The dfc-data repository supplies the canonical ship tonnage directly.
# These codes are used as the only faction-container filter tabs.
TONNAGE_CODES = ("L", "M", "H", "C")
BASE_SIZE_TO_TONNAGE_CODE = {
    20: "L",
    30: "L",
    40: "M",
    50: "H",
...
def normalise_tonnage_code(value, base_size=30) -> str:
    """Return the canonical L/M/H/C tonnage code from repository data.

    Ship, Hero and Famous Admiral records normally provide ``Tonnage``.
    Station and Deployable Feature records may omit it, so base size remains a
    deterministic fallback rather than dropping those records from all tabs.
    """
    raw = str(value or "").strip().upper()
    if raw in TONNAGE_ALIASES:
        return TONNAGE_ALIASES[raw]

    size = to_num(base_size)
    return BASE_SIZE_TO_TONNAGE_CODE.get(size, "L")
...
# A stripped-down self-contained script for Deployable Features.
# Includes Feature Card, Spikes, Activated, Firing Arcs + Scan, and Launch Range.
# No movement controls, health bar, status menu, backdrop, or 3D model toggle.
# HUD pivot raised to -170 (vs -340 for ships) since there is no stalk.

FEATURE_LUA_TEMPLATE = """\
--- FEATURE SCRIPT V1.5 (Deployable Feature - auto-generated by Gen18.py)
-------------------------------------------------------------------------------
-- Variables That Change Per Feature
local SHIP_ID = "{SHIP_ID}"
local Shiphealth = {Shiphealth}
local ShipbaseSize = {ShipbaseSize}
local ShipScan = {ShipScan}
local Shipcost = {Shipcost}
local SHIPCardURL = "{SHIPCardURL}"
local FIXED_IMAGE_URL = "{FIXED_IMAGE_URL}"

-- Full source record from dfc-data-main. This includes Profile.Special
-- keywords, Weapons, Load, Rules, Upgrades, Systems, AdmiralAbilities,
-- KnownShips, Flavour, and any future fields added by the data repository.
local SHIP_DATA_JSON = {ShipDataJSONLiteral}
local ShipData = {{}}

local function DecodeShipData()
    local ok, decoded = pcall(function() return JSON.decode(SHIP_DATA_JSON) end)
    ShipData = (ok and type(decoded) == "table") and decoded or {{}}
end

DecodeShipData()

-- Public accessors for future object XML and external object.call usage.
function GetShipData() return ShipData end
function GetShipDataJSON() return SHIP_DATA_JSON end
...
-- Range-line settings copied from the regular ship implementation.
...
local cachedLaunchLines = nil

local HiVis_BACKPLATE_WIDTH  = 527 * UI_SCALE
local HiVis_BACKPLATE_HEIGHT = 194 * UI_SCALE

-- Deployable Features do not use the normal ship stalk.  Their HUD and
-- top-down art therefore need their own object-space offsets.
local FEATURE_HUD_Z = -170
local FEATURE_IMAGE_WIDTH  = 200
local FEATURE_IMAGE_HEIGHT = 200
local FEATURE_IMAGE_POS_X  = 0
local FEATURE_IMAGE_POS_Y  = 0
local FEATURE_IMAGE_POS_Z  = -55
local FEATURE_IMAGE_ROT_X  = 90
local FEATURE_IMAGE_ROT_Y  = 270
local FEATURE_IMAGE_ROT_Z  = 90
...
Conditions = {{
    ...
    Aura = {{
        url        = "https://raw.githubusercontent.com/TemporalDistoriton/DropfleetTTS/main/Assets/UI/SHIPUI/Auranode2.png",
        active_url = "https://raw.githubusercontent.com/TemporalDistoriton/DropfleetTTS/main/Assets/UI/SHIPUI/Auranode2.png",
        color = "#FFFFFF", stacks = false
    }},
    Launch = {{
        url        = "https://raw.githubusercontent.com/TemporalDistoriton/DropfleetTTS/main/Assets/UI/SHIPUI/Launchoff.png",
        active_url = "https://raw.githubusercontent.com/TemporalDistoriton/DropfleetTTS/main/Assets/UI/SHIPUI/Launch.png",
        color = "#FFFFFF", stacks = false
    }},
    Activated = {{
...
local TOOLTIPS = {{
    ...
    Launch     = "Toggle 6-inch Launch Range",
    Fire       = "Fire Markers (+/-)",
...
-------------------------------------------------------------------------------
-- Firing arcs, scan range, and launch range
...
function generateFiringArcLines()
    ...
    local scale = getScaleFromBaseSize(ShipbaseSize)
    local arcLineLength = 16 * UNIT_SCALE / scale

    local function addAngleLines(angleDeg, lineColor)
        local right = forward:copy():rotateOver('y', angleDeg)
        local left  = forward:copy():rotateOver('y', -angleDeg)

        local p1 = right * arcLineLength
        local p2 = right * (-arcLineLength)
        local p3 = left  * arcLineLength
        local p4 = left  * (-arcLineLength)
        p1.y = LINE_Y_OFFSET
        p2.y = LINE_Y_OFFSET
        p3.y = LINE_Y_OFFSET
        p4.y = LINE_Y_OFFSET
        ...
        table.insert(cachedFiringArcLines, {{
            points = {{p1, p2}}, color = lineColor, thickness = 0.1
        }})
        table.insert(cachedFiringArcLines, {{
            points = {{p3, p4}}, color = lineColor, thickness = 0.1
        }})
    end

    -- Same arc colours and angles as regular ships.
...
function generateScanLines()
    ...
    local scale = getScaleFromBaseSize(ShipbaseSize)
    local scanRadius = math.max(0, tonumber(ShipScan) or 0) * UNIT_SCALE / scale
    if scanRadius <= 0 then return end
    ...
    for i = 0, segments do
        ...
        table.insert(circlePoints, Vector(
    ...
    table.insert(cachedScanLines, {{
        points = circlePoints, color = {{0.5, 1, 0.5}}, thickness = 0.1
    }})
end

function generateLaunchLines()
    cachedLaunchLines = {{}}

    local scale = getScaleFromBaseSize(ShipbaseSize)
    local launchRadius = 6 * UNIT_SCALE / scale
    local circlePoints = {{}}
    local segments = 64
    local step = 360 / segments
    for i = 0, segments do
        local angle = math.rad(i * step)
        table.insert(circlePoints, Vector(
            math.cos(angle) * launchRadius,
            LINE_Y_OFFSET,
            math.sin(angle) * launchRadius
        ))
    end

    table.insert(cachedLaunchLines, {{
        points = circlePoints, color = {{1, 0.6, 0}}, thickness = 0.15
    }})
end

local function appendRangeLines(target, source)
    if not source then return end
    for _, line in ipairs(source) do table.insert(target, line) end
end

function updateRangeVisuals()
    local finalLines = {{}}
    if (state.conditions.Aura or 0) > 0 then
        appendRangeLines(finalLines, cachedFiringArcLines)
        appendRangeLines(finalLines, cachedScanLines)
    end
    if (state.conditions.Launch or 0) > 0 then
        appendRangeLines(finalLines, cachedLaunchLines)
    end
    self.setVectorLines(finalLines)
end

function rebuildRangeVisuals()
    if (state.conditions.Aura or 0) > 0 then
        generateFiringArcLines()
        generateScanLines()
    else
        cachedFiringArcLines = nil
        cachedScanLines = nil
    end
...
function UI_ModifyAura(p, alt)
    ...
    state.conditions.Aura = ((state.conditions.Aura or 0) > 0) and 0 or 1
...
function UI_ModifyLaunch(p, alt)
    if alt == '-3' then return end
    state.conditions.Launch = ((state.conditions.Launch or 0) > 0) and 0 or 1
    if state.conditions.Launch > 0 then
        generateLaunchLines()
        print("[FFAA00]Launch range shown (6 inches)[-]")
    else
        cachedLaunchLines = nil
        print("[AAAAAA]Launch range hidden[-]")
    end
    updateRangeVisuals()
    SyncCondition("Launch")
end
...
function FeatureHUDConditions(color)
    ...
    local conds = {{
        ...
        HUDSingleCondition(color, "Aura",      LeftAlign+4.0,  TopRow, size),
        HUDSingleCondition(color, "Launch",    LeftAlign+5.25, TopRow, size),
    }}
...
function rebuildAssets()
    local assets = {{}}
    -- The feature's top-down art is an object UI asset, not a child model.
    -- Prefer the dedicated model image and fall back to the card front so a
    -- missing model image never leaves an invisible feature.
    local featureImageURL = FIXED_IMAGE_URL
    if not featureImageURL or featureImageURL == "" then
        featureImageURL = SHIPCardURL
    end
    if featureImageURL and featureImageURL ~= "" then
        assets[#assets+1] = {{ name="FeatureModelImage", url=featureImageURL }}
    end
...
function generateFeatureImagePanel()
    local imageURL = FIXED_IMAGE_URL
    if not imageURL or imageURL == "" then imageURL = SHIPCardURL end
    if not imageURL or imageURL == "" then return "" end

    return string.format([[
        <Panel id="FeatureFixedImagePanel"
               width="%d" height="%d"
               position="%d %d %d"
               rotation="%d %d %d"
               rectAlignment="MiddleCenter">
            <Image width="100%%" height="100%%"
                   image="FeatureModelImage"
                   preserveAspect="true"/>
        </Panel>
    ]],
    FEATURE_IMAGE_WIDTH, FEATURE_IMAGE_HEIGHT,
    FEATURE_IMAGE_POS_X, FEATURE_IMAGE_POS_Y, FEATURE_IMAGE_POS_Z,
    FEATURE_IMAGE_ROT_X, FEATURE_IMAGE_ROT_Y, FEATURE_IMAGE_ROT_Z)
end
...
function ui()
    ...
    -- Keep the feature art independent of player visibility, exactly as the
    -- normal ship fixed-image panel is.  This also prevents the art being
    -- duplicated once per connected player.
    local featureImagePanel = generateFeatureImagePanel()
...
function InitFromSpawner(params)
    ...
    if params.scan or params.shipScan then
        ShipScan = tonumber(params.scan or params.shipScan) or ShipScan
    end
...
function recoverState(save)
    ...
    state.conditions.Aura        = state.conditions.Aura        or 0
    state.conditions.Launch      = state.conditions.Launch      or 0
...
def build_ship_vars(row: dict) -> dict:
    ...
    return {
        ...
        'ShipDataJSON':      build_ship_data_json(row),
...
def build_lua_script_state(template_state_json: str, row: dict) -> str:
    ...
    state['ShipDataSchemaVersion'] = SHIP_DATA_SCHEMA_VERSION
    state['ShipDataRecordKind'] = str(row.get('Record Kind', 'Ship') or 'Ship')
...
def build_stripped_ship_obj(template: dict, row: dict, existing_guids: set) -> tuple:
    ...
    if is_deployable:
        # Build a deterministic, centred feature base instead of retaining
        # transform/material fields from whichever ship was used as template.
        transform = ship.setdefault('Transform', {})
        transform.update({
        ...
        mesh = ship.setdefault('CustomMesh', {})
        mesh.update({
            'MeshURL': DEPLOYABLE_MESH_URL,
            'DiffuseURL': '',
            'NormalURL': '',
            'ColliderURL': DEPLOYABLE_COLLIDER_URL,
            'Convex': True,
            'MaterialIndex': 0,
            'TypeIndex': 0,
            'CastShadows': True,
        })
        # Ship-template shaders can be tied to the original base material.
        # Removing them avoids carrying incompatible rendering data into the
        # dedicated feature mesh.
        mesh.pop('CustomShader', None)

        ship['ColorDiffuse']   = DEPLOYABLE_COLOR
        ship['MeasureMovement'] = False
        ship['ChildObjects']   = []   # remove stalk and ship-model attachment
        ship['AttachedDecals'] = []   # remove ship arc indicator decal
    else:
...
function buildScriptHeader(vars)
    ...
    local lines = {}
    local shipDataJSON = tostring(vars.ShipDataJSON or "{}")
    local shipDataEq = findSafeDelimiter(shipDataJSON)
    table.insert(lines, "--- SHIP SCRIPT V8.3 (Template-Reconstructed with Full Ship Data)")
    table.insert(lines, "-------------------------------------------------------------------------------")
    ...
    table.insert(lines, "")
    table.insert(lines, "-- Full source record from dfc-data-main")
    table.insert(lines, "local SHIP_DATA_JSON = [" .. shipDataEq .. "[" .. shipDataJSON .. "]" .. shipDataEq .. "]")
    table.insert(lines, "local ShipData = {}")
    table.insert(lines, "local function DecodeShipData()")
    table.insert(lines, "    local ok, decoded = pcall(function() return JSON.decode(SHIP_DATA_JSON) end)")
    table.insert(lines, '    ShipData = (ok and type(decoded) == "table") and decoded or {}')
    table.insert(lines, "end")
    table.insert(lines, "DecodeShipData()")
    table.insert(lines, "function GetShipData() return ShipData end")
    table.insert(lines, "function GetShipDataJSON() return SHIP_DATA_JSON end")
    table.insert(lines, "")
...
function onLoad(saved_data)
    ...
    for _, card in ipairs(savedShipCards) do
        ...
        if card.shipType    then card.shipType    = stripBrackets(card.shipType) end
...
function updateSave()
    ...
    for i, card in ipairs(savedShipCards) do
        ...
        if card.shipVars and card.shipObjJson then
            ...
            for _, key in ipairs({"SHIP_ID", "SHIPCardURL", "FIXED_IMAGE_URL", "TARGET_MESH_URL", "TARGET_DIFFUSE_URL", "ShipDataJSON"}) do
...
def build_embedded_data(card_entries: list, script_template: str) -> str:
    ...
    # 5. Per-container type catalogue gathered from the current repository data.
    # The Lua container can refresh this after manual Setup edits, but embedding
    # the initial catalogue keeps the generated save deterministic.
    available_types = sorted(
        {str(card.get("shipType", "Other") or "Other").strip() for card in card_entries},
        key=str.casefold,
    )
    lines.append("availableShipTypes = {")
    for ship_type in available_types:
        lines.append(f"  [====[{ship_type}]====],")
    lines.append("}")
    lines.append("")
    ...
    for card in card_entries:
        ...
        lines.append(f"    shipType = [====[{card['shipType']}]====],")
        ...
        if 'json' in card:
            ...
            for key in ('SHIP_ID', 'SHIPCardURL', 'FIXED_IMAGE_URL', 'TARGET_MESH_URL', 'TARGET_DIFFUSE_URL', 'ShipDataJSON'):
...
# Neutral/civilian records have historically appeared under several labels in
# source exports and the TTS art repository.  They all route to the Civilian
# container unless they are nested beneath one of the six real fleet folders.
NEUTRAL_DATA_FOLDER_NAMES = {
    "civilian", "civilians", "civilianships",
    "industrial", "industry", "ind",
    "mercenary", "mercenaries", "neutral", "common",
    "scenario", "scenarios", "scenarioships",
}
...
# Art filenames are resolved against the actual local RemasterShips directory.
# This avoids hard-coded one-off exceptions for Famous Admirals, Heroes and
# other special records whose artwork name contains extra class/type text.
ART_FILE_SUFFIXES = {
    "model": "_ModelImage.png",
    "card": "_CardFrontImage.png",
}

_ART_CATALOG = None
_ART_DIRECTORY = None
_ART_MISSING_DIRECTORY_WARNING_SHOWN = False
_ART_UNRESOLVED_WARNINGS: set[tuple[str, str]] = set()
...
def locate_dfc_data_directory() -> Path:
    ...
    raise DataImportError(
        ...
        f"  {candidate}\n"
        "Place dfc-data-main beside Gen19_DFC_Data.py."
    )
...
def canonical_art_name(value: str) -> str:
    """Return a comparison key for artwork basenames.

    The key treats straight/curly quotes and common dash characters as
    equivalent, while preserving the words and their order.
    """
...
def locate_remaster_ships_directory() -> Path | None:
    """Locate the repository's RemasterShips folder by walking upward.

    The generator normally lives below the repository root, for example:
    ``DropfleetTTS/Test Code/OneAsMany/Gen10_DFC_Data.py``.
    """
    checked = set()
    for base in (SCRIPT_DIRECTORY, *SCRIPT_DIRECTORY.parents):
        candidate = (base / "RemasterShips").resolve()
...
def load_art_catalog() -> dict:
    """Index the exact artwork basenames present in local RemasterShips."""
    global _ART_CATALOG, _ART_DIRECTORY
    global _ART_MISSING_DIRECTORY_WARNING_SHOWN

    if _ART_CATALOG is not None:
        return _ART_CATALOG

    root = locate_remaster_ships_directory()
    _ART_DIRECTORY = root
    _ART_CATALOG = {}

    if root is None:
        if not _ART_MISSING_DIRECTORY_WARNING_SHOWN:
            print(
                "  [ART] WARNING: Could not find a local RemasterShips folder. "
                "Artwork names will use schema-derived fallbacks."
            )
            _ART_MISSING_DIRECTORY_WARNING_SHOWN = True
        return _ART_CATALOG
    ...
    for path in root.rglob("*"):
        ...
        matched_kind = None
        matched_suffix = None
        for kind, suffix in ART_FILE_SUFFIXES.items():
        ...
        pdf_id = faction_pdf_id(relative.parts[0])
        basename = filename[:-len(matched_suffix)]
        faction_entries = _ART_CATALOG.setdefault(pdf_id, {})
        faction_entries.setdefault(basename, set()).add(matched_kind)

    total_bases = sum(len(entries) for entries in _ART_CATALOG.values())
    print(f"  [ART] Indexed {total_bases} artwork basename(s) from: {root}")
    for pdf_id, entries in sorted(_ART_CATALOG.items()):
        print(f"  [ART]   {pdf_id}: {len(entries)} basename(s)")

    return _ART_CATALOG


def _deduplicate_art_candidates(candidates) -> list[str]:
    """Return non-empty candidate names without canonical duplicates."""
    result = []
    seen = set()
    for candidate in candidates or []:
        candidate = str(candidate or "").strip()
        key = canonical_art_name(candidate)
...
def _best_art_entry(entries: dict[str, set[str]], names: list[str]) -> str | None:
    """Choose the strongest unambiguous asset from a list of actual basenames."""
    if not names:
        return None

    def rank(name: str):
        kinds = entries.get(name, set())
        # Prefer a basename with both card and model art, then the shorter name.
        return (len(kinds), -len(canonical_art_name(name)), name.casefold())

    ordered = sorted(set(names), key=rank, reverse=True)
    if len(ordered) == 1:
        return ordered[0]

    first_rank = rank(ordered[0])[:2]
    second_rank = rank(ordered[1])[:2]
...
def resolve_art_asset_name(
    ...
    *,
    ...
    """Resolve an artwork basename from the files that actually exist.

    Resolution order:
      1. Exact filename match, case-insensitive.
      2. Exact normalised match, allowing quote/dash variants.
      3. Unique prefix match, allowing assets to append a ship type.

    The first candidate remains the fallback when the local art repository is
    unavailable or no safe unique match can be made.
    """
    candidate_names = _deduplicate_art_candidates(candidates)
    fallback = candidate_names[0] if candidate_names else str(context or "Unknown Ship")

    catalog = load_art_catalog()
    entries = catalog.get(str(pdf_id or "").strip().upper(), {})
    if not entries:
        return fallback

    # Pass 1: raw case-insensitive exact matches.
    raw_map: dict[str, list[str]] = {}
    for actual in entries:
        raw_map.setdefault(actual.casefold(), []).append(actual)
    for candidate in candidate_names:
        chosen = _best_art_entry(entries, raw_map.get(candidate.casefold(), []))
        if chosen:
            if verbose or canonical_art_name(chosen) != canonical_art_name(fallback):
                print(
                    f"  [ART RESOLVE] {context or fallback!r}: "
                    f"{fallback!r} -> {chosen!r}"
                )
            return chosen

    # Pass 2: normalised exact matches.
    canonical_map: dict[str, list[str]] = {}
    for actual in entries:
        canonical_map.setdefault(canonical_art_name(actual), []).append(actual)
    for candidate in candidate_names:
        chosen = _best_art_entry(entries, canonical_map.get(canonical_art_name(candidate), []))
        if chosen:
            if verbose or canonical_art_name(chosen) != canonical_art_name(fallback):
                print(
                    f"  [ART RESOLVE] {context or fallback!r}: "
    ...
    # Pass 3: unique prefix match. This is what resolves records such as
    # "Agency - Bastion" to the actual "Agency - Bastion Battleship" asset.
    actual_keys = {actual: canonical_art_name(actual) for actual in entries}
    for candidate in candidate_names:
        candidate_key = canonical_art_name(candidate)
        prefix_matches = [
            actual
            for actual, actual_key in actual_keys.items()
            if actual_key.startswith(candidate_key + " ")
            or actual_key.startswith(candidate_key + " - ")
        ]
        chosen = _best_art_entry(entries, prefix_matches)
        if chosen:
            print(
                f"  [ART RESOLVE] {context or fallback!r}: "
                f"{fallback!r} -> {chosen!r}"
    ...
    warning_key = (str(pdf_id).upper(), canonical_art_name(fallback))
    if warning_key not in _ART_UNRESOLVED_WARNINGS:
        print(
            f"  [ART] WARNING: No unique local artwork match for "
            f"{context or fallback!r} in faction {str(pdf_id).upper()}. "
            f"Using fallback basename {fallback!r}."
        )
        _ART_UNRESOLVED_WARNINGS.add(warning_key)

    return fallback


def append_type_to_asset_name(base_name: str, type_text: str) -> str:
    """Append a ship type unless the base already ends with that type."""
    base_name = str(base_name or "").strip()
    type_text = str(type_text or "").strip()
    if not type_text:
        return base_name
    if canonical_art_name(base_name).endswith(canonical_art_name(type_text)):
        return base_name
    return f"{base_name} {type_text}".strip()
...
def patch_container_dual_filter_menu(lua_script: str) -> tuple[str, dict]:
    """Install the two-stage Tonnage/Type filter menu on a faction container.

    Behaviour:
      * View Ships opens a compact menu with Filter by Tonnage and Filter by Type.
      * Tonnage is the default and opens the L/M/H/C picker.
      * Type displays the Python-generated ship types as vertically stacked TTS
        object buttons beginning at the old View Ships button position.
      * The non-working Spawn from JSON button is removed, but its functions are
        deliberately left in the Lua source for later repair.
    """
    if not isinstance(lua_script, str):
        return lua_script, {}

    # Normalise MT save scripts to LF so patching is independent of the TTS\n    # save's CRLF/LF encoding. TTS accepts either form.
    lua_script = lua_script.replace("\r\n", "\n")

    changes = {
        'tonnage_categories': False,
        'state': False,
        'view_menu': False,
        'picker_mode_row': False,
        'filter_logic': False,
        'json_button_hidden': False,
        'save_fields': False,
    }

    patched, n = re.subn(
        r'CLASS_CATEGORIES\s*=\s*\{.*?\},',
    ...
    lua_script = patched
    changes['tonnage_categories'] = n > 0

    lua_script, _ = re.subn(
        r'TONNAGE_CATEGORIES\s*=\s*\{.*?\},',
        'TONNAGE_CATEGORIES = {},',
        lua_script,
        count=1,
        flags=re.DOTALL,
    ...
    # Remove the obsolete second Light/Medium/Heavy/Colossal XML row.
    loop_marker = 'for _, tonnage in ipairs(CONFIG.TONNAGE_CATEGORIES) do'
    loop_pos = lua_script.find(loop_marker)
    if loop_pos != -1:
        xml_marker = 'xml = xml .. [['
        pre_start = lua_script.rfind(xml_marker, 0, loop_pos)
        post_start = lua_script.find(xml_marker, loop_pos + len(loop_marker))
        if pre_start != -1 and post_start != -1:
            first_close = lua_script.find('</HorizontalLayout>', pre_start, loop_pos)
            second_close = lua_script.find('</HorizontalLayout>', post_start)
            if first_close != -1 and second_close != -1:
                first_close_end = first_close + len('</HorizontalLayout>')
                second_close_end = second_close + len('</HorizontalLayout>')
                lua_script = (
                    lua_script[:pre_start]
                    + lua_script[pre_start:first_close_end]
                    + lua_script[second_close_end:]
                )

    state_marker = '  sharedActiveTonnageTab = nil\n  sharedCardIndex = 1'
    state_replacement = """  sharedActiveTonnageTab = nil
  sharedFilterMode = "tonnage"
  sharedSelectedType = nil
  availableShipTypes = availableShipTypes or {}
  sharedCardIndex = 1"""
    if state_marker in lua_script:
        lua_script = lua_script.replace(state_marker, state_replacement, 1)
        changes['state'] = True

    view_pattern = re.compile(
        r'  function buttonClick_viewShips\(obj, player_color\).*?\n  end\n\n\n  function showShipPickerUI\(\)',
        re.DOTALL,
    )
    view_replacement = r"""  function refreshAvailableShipTypes()
      local seen = {}
      local refreshed = {}
  ...
  function uiBackToFilterMenu(player, value, id)
      self.UI.setXml("")
      sharedShipUIOpen = false
      createFilterModeMenu()
  end
  ...
  function showShipPickerUI()"""
    lua_script, n = view_pattern.subn(view_replacement, lua_script, count=1)
    changes['view_menu'] = n > 0

    tab_start = """      if activeTab and countCardsInCategory(activeTab) == 0 then
          activeTab = nil
          sharedActiveTab = nil
      end

      for _, category in ipairs(CONFIG.CLASS_CATEGORIES) do"""
    if tab_start in lua_script:
        loop_start = lua_script.index(tab_start)
        close_marker = """      xml = xml .. [[
              </HorizontalLayout>"""
        loop_end = lua_script.index(close_marker, loop_start)
        new_block = """      if sharedFilterMode == "type" then
          ...
          xml = xml .. [[
                  <Button id="backToFilterMenuButton"
                          onClick="]] .. selfGUID .. [[/uiBackToFilterMenu"
                          flexibleWidth="1"
                          minHeight="]] .. tabsRowHeight .. [["
                          preferredHeight="]] .. tabsRowHeight .. [["
                          color="rgba(0.15,0.15,0.15,1)"
                          textColor="]] .. cfg.TEXT_COLOR .. [["
                          fontSize="]] .. cfg.FONT_SIZE_CLASS_TABS .. [[">Back</Button>
          ]]
...
"""
        lua_script = lua_script[:loop_start] + new_block + lua_script[loop_end:]
        changes['picker_mode_row'] = True

    filter_pattern = re.compile(
        r'  function getFilteredCards\(activeTab, activeTonnageTab, searchFilter\).*?\n  end\n\n  function countCardsInCategory',
        re.DOTALL,
    )
    filter_replacement = r"""  function getFilteredCards(activeTab, activeTonnageTab, searchFilter)
      local filtered = {}
  ...
  function countCardsInCategory"""
    lua_script, n = filter_pattern.subn(filter_replacement, lua_script, count=1)
    changes['filter_logic'] = n > 0

    lua_script = lua_script.replace(
        'local activeTonnageTab = sharedActiveTonnageTab',
        'local activeTonnageTab = nil\n      sharedActiveTonnageTab = nil',
        1,
    )

    save_marker = 'embeddedData = embeddedData .. "    shipTonnage = [====[" .. (card.shipTonnage or "Light") .. "]====],\\n"'
    base_scale_marker = 'embeddedData = embeddedData .. "    baseScale = " .. (card.baseScale or 1) .. ",\\n"'
    save_pair = save_marker + "\n              " + base_scale_marker
    if save_pair in lua_script:
        lua_script = lua_script.replace(
            save_pair,
            save_marker
            + '\n              embeddedData = embeddedData .. "    shipType = [====[" .. (card.shipType or card.description or "Other") .. "]====],\\n"'
        ...
        changes['save_fields'] = True

    extract_marker = '              name = cardObj.getName() or "Unknown Ship",\n              json = nil,'
    if extract_marker in lua_script:
        lua_script = lua_script.replace(
            extract_marker,
    ...
    json_button_pattern = re.compile(
        r'\n\s*-- Spawn from JSON button\n\s*createButton_helper\(\{.*?click_function = "buttonClick_spawnFromJSON".*?\n\s*\}\)\n',
        re.DOTALL,
    )
    lua_script, n = json_button_pattern.subn('\n', lua_script, count=1)
    changes['json_button_hidden'] = n > 0

    return lua_script, changes
...
def base_row(
    ...
    *,
    ...
    art_context: str = "",
    art_verbose: bool = False,
    filter_type=None,
    record_kind: str = "Ship",
    source_data=None,
) -> dict:
    ...
    candidates = asset_candidates if asset_candidates is not None else [asset_name]
    resolved_asset_name = resolve_art_asset_name(
        pdf_id,
        candidates,
        context=art_context or str(display_name),
        verbose=art_verbose,
    )
    ...
    row = {
        ...
        'Filter Type':      str(data.get('Type', '') if filter_type is None else filter_type).strip() or 'Other',
        ...
        'PDF ID':           pdf_id,
        'Model Image':      art_url(pdf_id, resolved_asset_name, '_ModelImage.png'),
        'Card Image':       art_url(pdf_id, resolved_asset_name, '_CardFrontImage.png'),
        '3D MODEL FILE':    'None',
        '3D MODEL TEXTURE': 'None',
        'Record Kind':      str(record_kind or 'Ship'),
        'Source Data':      copy.deepcopy(data if source_data is None else source_data),
        'Source File':      '',
        'Source Folder':    '',
    }
    row['Tonnage'] = normalise_tonnage_code(data.get('Tonnage'), row['Size'])
    return row
...
def ship_row(data: dict, pdf_id: str) -> dict:
    ...
    candidates = [
        name,
        append_type_to_asset_name(name, ship_type),
    ]
    return base_row(
        data,
        pdf_id,
        ...
        art_context=f"Ship {name}",
    )


def famous_admiral_row(data: dict, pdf_id: str) -> dict:
    """Build a Famous Admiral row and resolve its actual artwork filename."""
    ...
    ship_type = str(data.get('Type', '')).strip()
    class_asset = f"{admiral} - {ship_class}" if ship_class else admiral
    candidates = [
        class_asset,
        append_type_to_asset_name(class_asset, ship_type),
        f"{admiral} - {ship_type} {ship_class}".strip(" -"),
        admiral,
    ]
    ...
    return base_row(
        ...
        description,
        asset_candidates=candidates,
        art_context=f"Famous Admiral {admiral}",
        art_verbose=True,
        filter_type=ship_type,
        record_kind="Famous Admiral",
    )

def hero_row(data: dict, pdf_id: str) -> dict:
    """Build a Hero row and resolve the exact local artwork basename."""
    ...
    candidates = [
        display_name,
        append_type_to_asset_name(display_name, ship_type),
        f"{hero} - {ship_type} {ship_name}".strip(" -"),
        ship_name,
        hero,
    ]
...
def deployable_feature_row(data: dict, pdf_id: str) -> dict:
    ...
    return base_row(
        ...
        filter_type='Deployable Feature',
        record_kind='Deployable Feature',
    )
...
def station_row(data: dict) -> dict:
    ...
    return base_row(
        ...
        profile=profile,
        thrust=0,
        filter_type=str(data.get('Type', 'Space Station')),
        record_kind='Station',
    )
...
def load_index_records(
    ...
    for filename in _index_filenames(index, key):
        ...
        try:
            ...
            set_source_path_metadata(row, data_path)
            rows.append(row)
...
def civilian_ship_row(data: dict, source_path: Path) -> dict:
    ...
    row = base_row(
        ...
        points=points,
        base_size=base_size,
        record_kind="Civilian Ship",
        source_data=data,
    )
    return set_source_path_metadata(row, source_path)
...
def infer_pdf_id_from_source_path(data_path: Path, data_directory: Path) -> str:
    """Infer the owning TTS faction from any path below the Data repository.

    First prefer one of the six actual fleet folder names anywhere in the path.
    If none is present, neutral/civilian/industrial/scenario-style folders route
    to CIV.  Completely unknown locations also route to CIV so a valid ship is
    never silently lost merely because the repository gained a new wrapper
    directory.
    """
    try:
        relative_parts = data_path.resolve().relative_to(data_directory.resolve()).parts[:-1]
    except (ValueError, OSError):
        relative_parts = data_path.parent.parts

    # Fleet ownership wins over generic nested labels such as "Scenarios".
    for part in relative_parts:
        mapped = FACTION_TO_PDF_ID.get(str(part).strip().casefold())
        if mapped and mapped != CIVILIAN_FALLBACK_ID:
            return mapped

    for part in relative_parts:
        cleaned = normalised_folder_name(part)
        mapped = FACTION_TO_PDF_ID.get(str(part).strip().casefold())
        if mapped:
            return mapped
        if cleaned in NEUTRAL_DATA_FOLDER_NAMES:
            return CIVILIAN_FALLBACK_ID

    return CIVILIAN_FALLBACK_ID


def is_direct_ship_like_record(data) -> bool:
    """Return True only for dictionaries that look like complete playing pieces."""
    if not isinstance(data, dict):
        return False

    profile = data.get('Profile')
    if data.get('Admiral') and data.get('Class') and isinstance(profile, dict):
        return True
    if data.get('Hero') and (data.get('Ship') or data.get('Class')) and isinstance(profile, dict):
        return True
    if data.get('Fleet') and data.get('Name') and isinstance(profile, dict):
        return True
    if data.get('Class') and isinstance(profile, dict):
        return True

    # Deployable Feature schema: Profile contains PTS/Type rather than ship stats.
    if isinstance(profile, dict) and profile.get('Type') and 'PTS' in profile:
        return ('Weapons' in data or 'Load' in data or 'Rules' in data)

    return False


def iter_embedded_ship_records(payload):
    """Yield complete ship-like dictionaries from arbitrary JSON wrappers.

    The normal repository stores one ship per JSON file, but this intentionally
    tolerates future wrapper/list exports.  Once a complete ship record is found
    we do not recurse into it, preventing nested weapon/rule dictionaries from
    being mistaken for separate records.
    """
    if is_direct_ship_like_record(payload):
        yield payload
        return

    if isinstance(payload, list):
        for item in payload:
            yield from iter_embedded_ship_records(item)
        return

    if isinstance(payload, dict):
        for value in payload.values():
            if isinstance(value, (dict, list)):
                yield from iter_embedded_ship_records(value)


def build_discovered_row(data: dict, data_path: Path, data_directory: Path) -> dict | None:
    """Build a row for a ship discovered outside the normal index paths."""
    if not isinstance(data, dict):
        return None

    pdf_id = infer_pdf_id_from_source_path(data_path, data_directory)

    # Specialised records first because Famous Admirals and Heroes can also
    # contain Class.
    if data.get('Admiral') and data.get('Class') and isinstance(data.get('Profile'), dict):
        return famous_admiral_row(data, pdf_id)

    if data.get('Hero') and (data.get('Ship') or data.get('Class')) and isinstance(data.get('Profile'), dict):
        return hero_row(data, pdf_id)
    ...
    if (
        ...
        and 'PTS' in profile
        and ('Weapons' in data or 'Load' in data or 'Rules' in data)
    ):
        return deployable_feature_row(data, pdf_id)

    return None


def load_repository_wide_unclaimed_ship_files(
    data_directory: Path,
    loaded_paths: set[Path],
) -> list:
    """Final recovery pass over *every* JSON file below Data.

    This is deliberately independent of folder/index naming.  It catches valid
    ships placed at Data root, under Industrial/Mercenary/Scenario folders, or
    inside wrapper/list JSON exports.  Index files are skipped because the ship
    data files they reference are scanned directly.
    """
    rows = []
    recovered_files = 0
    recovered_records = 0
    ...
    all_json = sorted(
        (p for p in data_directory.rglob('*')
         if p.is_file() and p.suffix.casefold() == '.json'),
        key=lambda p: str(p).casefold(),
    )

    for data_path in all_json:
        resolved = data_path.resolve()
        if resolved in loaded_paths or data_path.name.startswith('_'):
            continue

        payload = load_json(data_path)
        records = list(iter_embedded_ship_records(payload))
        if not records:
            continue

        file_rows = []
        for record in records:
            row = build_discovered_row(record, data_path, data_directory)
            if row is None:
                continue
            set_source_path_metadata(row, data_path)
        ...
        recovered_files += 1
        recovered_records += len(file_rows)

        for row in file_rows:
            try:
                rel = data_path.resolve().relative_to(data_directory.resolve())
            except ValueError:
                rel = data_path
            print(
                f"  [GLOBAL RECOVERY] {rel}: "
                f"{row.get('Name')} | {row.get('Type')} -> {row.get('PDF ID')}"
            )

    print(
        f"  Repository-wide recovery: {recovered_records} record(s) "
        f"from {recovered_files} previously unclaimed JSON file(s)"
    )
    return rows
...
def build_unindexed_row(data: dict, source_folder: Path) -> dict | None:
    ...
    if data.get('Hero') and (data.get('Ship') or data.get('Class')):
...
def read_ship_rows_from_data(data_directory: Path) -> list:
    ...
    # Final repository-wide recovery.  Do not assume every playable ship lives
    # directly beneath one of the known top-level fleet/Civilian directories.
    # This catches root-level files and newer neutral categories such as
    # Industrial, Mercenary, Scenario, or wrapper/list exports.
    global_recovered = load_repository_wide_unclaimed_ship_files(
        data_directory, loaded_paths
    )
    rows.extend(global_recovered)
    if global_recovered:
        source_counts['[GLOBAL RECOVERY]'] = len(global_recovered)
...
def main():
    ...
    # Apply the dual Tonnage/Type filter menu to every faction container,
    # including currently empty containers, and hide Spawn from JSON.
    for pdf_id, tile_idx in faction_tiles.items():
        container = save_data['ObjectStates'][tile_idx]
        lua_script = container.get('LuaScript', '')

        lua_script, xml_patch_applied = patch_container_xml_escaping(lua_script)
        lua_script, filter_patch = patch_container_dual_filter_menu(lua_script)
        container['LuaScript'] = lua_script

        if xml_patch_applied:
            print(f"  Applied XML image-URL escaping to '{container['Nickname']}'")
        if filter_patch.get("view_menu") and filter_patch.get("filter_logic"):
            print(f"  Installed Tonnage/Type filter menu in '{container['Nickname']}'")
        if filter_patch.get("json_button_hidden"):
            print(f"  Hid Spawn from JSON button in '{container['Nickname']}'")
    ...
    for i, row in enumerate(rows):
        ...
        if DEBUG_DATA_IMPORT:
            payload = build_ship_data_payload(row)
            weapons_count = len(payload.get('Weapons', []) or []) if isinstance(payload, dict) else 0
            rules_count = len(payload.get('Rules', []) or []) if isinstance(payload, dict) else 0
            keywords_count = len((payload.get('Profile', {}) or {}).get('Special', []) or []) \
                if isinstance(payload.get('Profile', {}), dict) else 0
            print(
                f"  [DATA] {ship_name}: kind={row.get('Record Kind', 'Ship')}, "
                f"weapons={weapons_count}, keywords={keywords_count}, rules={rules_count}"
            )
        ...
        if is_deployable:
            ...
            if DEBUG_DATA_IMPORT:
                print(
                    f"  [FEATURE] {ship_name}: base={int(base_size) if base_size else 0} mm, "
                    f"model art={row.get('Model Image', '')!s}"
                )
            ...
            card_entry = {
                ...
                "imageURL":    card_url,
                "tonnage":     normalise_tonnage_code(row.get('Tonnage'), base_size),
                "shipTonnage": detect_ship_tonnage(base_size),
                "shipType":    str(row.get('Filter Type') or ship_type or 'Other').strip(),
                "baseScale":   1,
    ...
    for pdf_id, card_entries in groups.items():
        ...
        # Build new embedded data and report the source Types gathered for
        # this specific faction container.
        gathered_types = sorted(
            {str(card.get("shipType", "Other") or "Other") for card in card_entries},
            key=str.casefold,
        )
        print(f"  {container['Nickname']} types ({len(gathered_types)}): "
              + ", ".join(gathered_types))
