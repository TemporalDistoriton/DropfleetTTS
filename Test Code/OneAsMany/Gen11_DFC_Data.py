#!/usr/bin/env python3
"""
TTS Ship Generator v11 — Template-Optimised (with Spawner Tiles)
================================================================
...
SCRIPT_VERSION = "Gen11 source-tonnage tabs build 2026-08-03"
...
def patch_container_source_tonnage_tabs(lua_script: str) -> tuple[str, bool]:
    """Replace the old class and base-size tab rows with one L/M/H/C row.

    The first existing tab row is retained and repurposed.  The second legacy
    Light/Medium/Heavy/Colossal row is removed from the generated XML, while
    its callbacks and compatibility fields may remain harmlessly in Lua.
    """
    if not isinstance(lua_script, str):
        return lua_script, False
    ...
    # Repurpose the first tab row.  Match only the compact CONFIG assignment,
    # not the later loops that consume it.
    lua_script = re.sub(
    ...
    # This collection is no longer rendered, but empty it to prevent an old
    # callback or future template variation from recreating duplicate tabs.
    lua_script = re.sub(
    ...
    # Remove the complete second tab-row XML block.  We locate it relative to
    # its unique loop marker rather than depending on exact indentation.
    ...
    # No hidden secondary filter should survive from a previous UI state.
    ...
    return lua_script, lua_script != original
...
def main():
    ...
    # Apply UI compatibility patches to every faction container, including any
    # currently empty container.  This leaves one filter row: L / M / H / C.
    for pdf_id, tile_idx in faction_tiles.items():
        ...
        lua_script, tonnage_patch_applied = patch_container_source_tonnage_tabs(lua_script)
        container['LuaScript'] = lua_script

        if xml_patch_applied:
            print(f"  Applied XML image-URL escaping to '{container['Nickname']}'")
        if tonnage_patch_applied:
            print(f"  Replaced class tabs with L/M/H/C tabs in '{container['Nickname']}'")
